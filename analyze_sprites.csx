...
using (Bitmap spriteImg = new Bitmap(spriteSheetPath))
{
    ...
    for (int i = 0; i < 4; i++)
    {
        Bitmap bmp = new Bitmap(refFrameW, refFrameH);
    ...
    double CompareFrames(Bitmap a, Bitmap b)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                ...
                Color pb = b.GetPixel(x + offsetX, y + offsetY);
    ...
    for (int row = 0; row < rows; row++)
    {
        ...
        for (int col = 0; col < 4; col++)
        {
            ...
            using (Bitmap frame = new Bitmap(spriteFrameW, spriteFrameH))
            {
                ...
                {
                    double diff = CompareFrames(refFrames[col], frame);
                    frameDiffs.Add(diff);
                    rowTotal += diff;
                }
                
                for (int py = 0; py < spriteFrameH; py++)
                {
                    for (int px = 0; px < spriteFrameW; px++)
                    {
                        Color p = frame.GetPixel(px, py);
                        if (p.A > 10)
                        {
                            totalNonTransparent++;
                            if (px < spriteFrameW / 2) leftNonTransparent++;
                            else rightNonTransparent++;
                            if (py < spriteFrameH / 2) topNonTransparent++;
        ...
        rowData.Add(new {
            ...
            Y = yOff,
            AvgDiff = avgScore,
            FrameDiffs = string.Join(", ", frameDiffs),
            LeftBias = leftBias,
            TopBias = topBias,
            PixelCount = totalNonTransparent / 4,
            AnimType = animType,
            PixelHeight = pixelHeight
        });
        
        Console.WriteLine($"{row,4} | {yOff,5} | {avgScore,8:F2} | {string.Join(", ", frameDiffs.Select(d => d.ToString("F2")))}");
    }
    
    Console.WriteLine();
    Console.WriteLine("=== 最佳匹配结果 ===");
    var best = rowData.First(r => r.Row == bestRow);
    ...
    foreach (var r in rowData)
    {
        ...
        Console.WriteLine($"行 {r.Row} (Y={r.Y}, 差异={r.AvgDiff:F2}): {r.AnimType} - {facing}, {pose} (像素数:{r.PixelCount}, 高度:{r.PixelHeight})");
    }
